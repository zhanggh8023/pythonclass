...
class DouYin(object):
    def __init__(self, width = 500, height = 300):
        ...
        self.headers['user-agent'] = hds[random.randint(0,len(hds)-1)]['User-Agent']

    def getToken(self):
        print('gettoken')
        #req = requests.get('http://api.appsign.vip:2688/token/douyin/version/2.7.0').json()
        req = '5735297261d21a0634e772de162bab56'
        return self.save_json('douyin_token.txt', req)

    def getDevice(self):
        print('133333331')
#         req = requests.get('http://api.appsign.vip:2688/douyin/device/new/version/2.7.0')
...
# #         print(req)
    ...
    def getSign(self, token, query):
        req = requests.post('http://api.appsign.vip:2688/sign', json={'token': token, 'query': query}).json()
        try:
            while req['message']:
    ...
    def get_video_urls(self, user_id, type_flag='f'):
        ...
        sign = {'mas':'0119eda7ed872d77623e814374d718739fececcc0c864c2666c61c',
            'as' : 'a165ac048614ec754a8477',
            'ts' : '1548410192'
            }
        while not sign:
            self.getToken()
