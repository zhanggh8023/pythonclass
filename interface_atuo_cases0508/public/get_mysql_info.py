...
class getMysqlInfo:
    ...
    def auto_insert(self):
        ...
        cursor = cnn.cursor()
        # 执行SQL语句
        cur = cursor.execute("select * from autotest.znkf")
        # 查看数据库数据
        ...
        for i in range(len(data)):
            ...
            list_1[data[i][0]] = ''
        table_sql = "INSERT INTO znkf (" + ','.join(data_1) + ")" + " VALUES " + "(" + ','.join(test) + ")"
...
if __name__ == '__main__':
    sql_result=getMysqlInfo(Allpath.db_conf_path, 'config').get_mysql_info("select * from full_quality_check_rule where rule_name=%s ORDER BY rule_id DESC","test_zgh",1)
